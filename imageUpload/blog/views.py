...
from .forms import PostForm, CommentForm, CreateUserForm, PostForm_summernote
...
def post_read(request,pk):
    post = get_object_or_404(Post, id=pk)
    multiFile = get_object_or_404(summer_model.Attachment, id = pk)
    form = CommentForm()
...
def new_post(request):
    # form = PostForm_summernote()
    # return render(request, 'blog/post_create_summernote.html', {'form': form})
    if request.method == 'POST':
        Post_form = PostForm_summernote(request.POST)

        if Post_form.is_valid():
            post = Post_form.save(commit=False)
            post.user = request.user #request의 사용자 정보 알아와서 사용자가 없다면 익명처리 해주려고
            post.save()
            return HttpResponse('post생성완료')
        
    else:
        Post_form = PostForm_summernote()

    return render(request, 'blog/post_create_summernote.html', {
        'form':Post_form,
    }) 
